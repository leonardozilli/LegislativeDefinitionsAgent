...
agent = LegalDefAgent(model=models._get_model('groq'))
agent_graph_runnable = agent.graph

def invoke_graph(st_messages, callables):
    if not isinstance(callables, list):
        raise TypeError("callables must be a list")
    return agent_graph_runnable.invoke({"messages": st_messages}, config={"callbacks": callables, "configurable": {"thread_id": f"{st.session_state.thread_id}"}})
