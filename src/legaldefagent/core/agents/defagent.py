"""Production legal definition agent with user-friendly output formatting."""
...
def build_agent_graph(tools: list = []) -> StateGraph:
    ...
    agent.add_node("supervisor", partial(call_model, tools=tools, instructions=INSTRUCTIONS))
    agent.add_node("tools", ToolNode(tools))
    agent.add_node("state_cleanup", state_cleanup)

    agent.set_entry_point("supervisor")
    agent.add_conditional_edges("supervisor", pending_tool_calls, {"tools": "tools", "done": "state_cleanup"})
...
agent_graph = build_agent_graph(tools=[definition_search])

definitions_agent = agent_graph.compile(checkpointer=MemorySaver())
