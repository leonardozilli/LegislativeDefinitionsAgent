import datetime
import logging
from typing import Annotated, Dict, Optional, Tuple, List
...
def get_retriever():
    ...
    if _RETRIEVER is None:
        _RETRIEVER = retriever_.setup_retriever(k=10)
    return _RETRIEVER
...
def query_vectorstore(query: str) -> List[Dict]:
    ...
    retrieved_definitions = retriever_instance.invoke(query)
    return utils.docs_list_to_json_list(retrieved_definitions)


def get_relevant_definitions_id(config: RunnableConfig, retrieved_definitions: list[Dict], question: str) -> List[int]:
...
def filter_definitions_by_jurisdiction(definitions: list[Dict], jurisdiction: str) -> list[Dict]:
    """Filter definitions by jurisdiction."""
    return [
        definition
        for definition in definitions
        if settings.collections[definition["metadata"]["dataset"]].jurisdiction == jurisdiction
    ]


def get_definition_timeline(definition: Dict) -> Optional[list[Dict[str, str]]]:
...
async def generate_definition(
    ...
    example_definitions: Optional[list[str]],
) -> Dict:
    ...
    EU_ROLE = "You are a legal expert specialized in drafting legal definitions for European Union jurisdiction. Your job is to draft a legally sound definition of a specified term suitable for use in legislation and consistent with the EU legal framework."
    IT_ROLE = "You are a legal expert specialized in drafting legal definitions for Italian jurisdiction. Your job is to draft a legally sound definition of a specified term suitable for use in legislation and consistent with the Italian legal framework."
    ...
    prompt = PromptTemplate(
        ...
        \n- {examples}\n
        """,
        input_variables=["jurisdiction_role", "question", "definiendum", "examples"],
        partial_variables={"format_instructions": parser.get_format_instructions()},
    )
    ...
    await Task("Generate definition").start(
        data={
            "input": prompt.invoke(
                {
                    "question": question,
                    "jurisdiction_role": EU_ROLE
                    if jurisdiction == "EU"
                    else IT_ROLE
                    if jurisdiction == "IT"
                    else DEFAULT_ROLE,
                    "definiendum": definiendum,
                    "examples": "\n- ".join(set(example_definitions)) if example_definitions else "",
                }
            )
        },
        config=config,
    ...
    response = chain.invoke(
        {
            "jurisdiction_role": EU_ROLE if jurisdiction == "EU" else IT_ROLE if jurisdiction == "IT" else DEFAULT_ROLE,
            "question": question,
            "definiendum": definiendum,
            "examples": "\n- ".join(set(example_definitions)) if example_definitions else "",
        }
...
def parse_jurisdiction(jurisdiction: Optional[str]) -> bool:
    return jurisdiction in ["EU", "IT"]
...
async def definition_search(
    ...
    Args:
        ...
        jurisdiction: Optional jurisdiction context to narrow search. Accepts 'EU' or 'IT'.
    ...
    """
    logger.info(f"Searching for definitions: {definiendum=}, {question=}, {jurisdiction=}, {date_filters=}")
    ...
    if jurisdiction and parse_jurisdiction(jurisdiction):
        logger.info(f"Filtering by jurisdiction: {jurisdiction}")
        await Task("Filter by jurisdiction").start(data={"input": relevant_definitions}, config=config)
        relevant_definitions = filter_definitions_by_jurisdiction(relevant_definitions, jurisdiction)
        await Task("Filter by jurisdiction").finish(
            result="success", data={"output": relevant_definitions}, config=config
        )

    logger.info(f"{len(relevant_definitions)} definitions left after filtering by jurisdiction")

    if not relevant_definitions:
        logger.info("No relevant definitions found. Generating definition...")
        examples = [str(definition.get("definition_text")) for definition in relevant_definitions]
        await Task("Retrieve definition").finish(result="success", data={"output": ""}, config=config)
        return await generate_definition(config, question, definiendum, jurisdiction, examples)
    ...
    definitions_with_timeline: list[Dict] = []
    ...
    if not definitions_with_timeline:
        ...
        examples = [str(definition.get("definition_text", None)) for definition in relevant_definitions]
        await Task("Retrieve definition").finish(result="success", data={"output": ""}, config=config)
        return await generate_definition(config, question, definiendum, jurisdiction, examples)
    ...
    picked_definitions = await pick_definition(config, definitions_with_timeline, question, definiendum)
    ...
    if len(picked_definitions_ids) == 0:
        examples = [str(definition.get("definition_text")) for definition in definitions_with_timeline]
        return {
            "generated_definition": await generate_definition(config, question, definiendum, jurisdiction, examples)
        }
