...
def filter_existing_definitions(definiendum, definitions):
    """Filter out definitions that exactly match the definiendum to avoid retrieving them as relevant."""
    return [
        definition
        for definition in definitions
        if utils.camelcase_to_spaces(definition.metadata["definiendum_label"]) != definiendum
    ]


def query_vectorstore(query: str, definiendum: str) -> list[Dict]:
    """Query vector store for relevant definitions."""
    retriever_instance = get_retriever()
    retrieved_definitions = retriever_instance.invoke(query)
    retrieved_definitions = filter_existing_definitions(definiendum, retrieved_definitions)
    retrieved_definitions = retrieved_definitions[:10]
...
def filter_definitions_by_jurisdiction(definitions: list[Dict], jurisdiction: str) -> list[Dict]:
    """Filter definitions by jurisdiction type."""
    return [
        definition
        for definition in definitions
...
async def generate_definition(
    ...
    example_definitions: Optional[list[str]] = None,
) -> Dict:
    ...
    response = chain.invoke(
        {
            ...
            "question": question,
            "definiendum": definiendum,
            "examples": "\n- ".join(set(example_definitions)) if example_definitions else "N/A",
        }
    )
...
async def definition_search(
    ...
    relevant_definitions = query_vectorstore(question, definiendum)
    ...
    if len(picked_definitions_ids) == 0:
        examples = list(set(str(definition.get("definition_text")) for definition in relevant_definitions))
        return {
            "generated_definition": await generate_definition(config, question, definiendum, jurisdiction, examples)
        }

    answer_def = [
        definition for definition in relevant_definitions if definition["metadata"]["id"] in picked_definitions_ids
    ]
