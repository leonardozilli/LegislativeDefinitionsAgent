...
def doc_to_json(doc: Document) -> Dict[str, str]:
    """Convert LangChain Document to JSON format."""
    json_doc = doc.to_json().get("kwargs")
    if not json_doc:
        raise ValueError("Document JSON conversion failed, no 'kwargs' found.")
...
def filter_documents_by_date(docs: List[Document], date_filters: Optional[Tuple[str, str]]) -> List[Document]:
    ...
    for doc in docs:
        doc_date = parse_date(extract_date_from_uri(doc.metadata["frbr_expression"]))
...
def format_definitions_dict(data: List[Dict[str, Any]], include_keywords: bool = True) -> str:
    ...
    for entry in data:
        ...
        if include_keywords and "keywords" in entry:
            parts.extend(["Keywords:", *[f"  - {kw}" for kw in entry["keywords"]]])
...
def get_available_vllm_models(host, port) -> List[str]:
    """Retrieve available VLLM models from the VLLM endpoint."""
    import requests

    try:
        response = requests.get(f"http://{host}:{port}/v1/models", timeout=2.0)
        response.raise_for_status()
        data = response.json()
        model_list = [model["id"] for model in data.get("data", [])]
        return model_list
    except requests.RequestException as e:
        raise RuntimeError(f"Failed to retrieve VLLM models: {e}") from e
