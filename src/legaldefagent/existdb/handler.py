...
class ExistDBHandler:
    ...
    def __init__(self, settings: dict):
        self.xdb_config = settings.EXIST_CONFIG
        self.db = db.ExistDB(
            server_url=f"http://{self.xdb_config.EXISTDB_HOST}:{self.xdb_config.EXISTDB_PORT}/exist/",
            username=self.xdb_config.EXISTDB_USER,
            password=self.xdb_config.EXISTDB_PASSWORD,
        )
    ...
    def get_all_definitions(self, dataset: str):
        QUERY = r"""
                xquery version "3.1";
                declare namespace akn = "{namespace}";
