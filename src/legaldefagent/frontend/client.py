...
class AgentClient:
    ...
    def retrieve_info(self) -> None:
        try:
            ...
            raise AgentClientError(f"Error getting service info from {self.base_url}/info: {e}")
    ...
    async def ainvoke(self, message: str, model: str | None = None, thread_id: str | None = None) -> ChatMessage:
        ...
        request = UserInput(message=message, thread_id=thread_id, model=cast(AllModelEnum | None, model))
    ...
    def invoke(self, message: str, model: str | None = None, thread_id: str | None = None) -> ChatMessage:
        ...
        if model:
            request.model = cast(AllModelEnum | None, model)
        try:
            response = httpx.post(
                f"{self.base_url}/{self.agent}/invoke",
                json=request.model_dump(),
