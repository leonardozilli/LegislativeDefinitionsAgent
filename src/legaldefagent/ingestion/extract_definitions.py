...
class DefinitionExtractor:
    ...
    def extract_from_local(self, input_dir: Path) -> pl.DataFrame:
    ...
    def parse_xml(self, xml_file: Path, dataset: str) -> Optional[List[Dict]]:
        try:
            ...
            root = tree.getroot()
            nsmap: Dict[str, str] = {"akn": str(self.collections[dataset].namespace)}

            frbr_work = root.find(".//akn:FRBRWork", nsmap).find(".//akn:FRBRthis", nsmap).attrib.get("value", "")
            frbr_expression = (
                root.find(".//akn:FRBRExpression", nsmap).find(".//akn:FRBRthis", nsmap).attrib.get("value", "")
            )
            keywords = root.findall(".//akn:keyword", nsmap)

            definitions_el = root.findall(".//akn:definitions", nsmap)
            if not definitions_el:
                return None

            definitions = []
            for definition in root.findall(".//akn:definition", nsmap):
                try:
                    definiendum, definiens, references, full_def = self._parse_definition(definition, root, nsmap)
                    if definiendum:
                        definitions.append(
                            {
                                "def_n": definition.find(".//akn:definitionHead", nsmap).attrib.get("href", ""),
                                "label": definition.find(".//akn:definitionHead", nsmap).attrib.get("refersTo", ""),
                                "definiendum": self._clean_definiendum(definiendum),
                                "definiens": self._clean_definiens(definiens),
                                "full_definition": self._clean_full_def(full_def) if full_def else None,
    ...
    def _parse_definition(
        self, definition, root, namespaces
    ) -> Tuple[Optional[str], str, List[Optional[str]], Optional[str]]:
        """Extract components from a single definition."""
        definition_head = definition.find(".//akn:definitionHead", namespaces)
        definition_body_elements = definition.findall(".//akn:definitionBody", namespaces)

        definiendum_id = definition_head.attrib.get("href", "").lstrip("#")
        try:
            definiendum = root.find(f".//akn:def[@eId='{definiendum_id}']", namespaces).text
        except AttributeError:
            definiendum = None

        try:
            full_def = "".join(root.find(f".//*[@defines='#{definiendum_id}']", namespaces).itertext())
        except Exception:
            full_def = None
        ...
        for body in definition_body_elements:
            body_text, body_refs = self._extract_body_and_references(body, root, namespaces)
            if body_text and body_refs:
    ...
    @staticmethod
    def _extract_body_and_references(body, root, namespaces) -> Tuple[Optional[str], Optional[List[str]]]:
        """Extract text and references from definition body."""
        body_id = body.attrib.get("href", "").lstrip("#")
        body_element = root.find(f".//akn:defBody[@eId='{body_id}']", namespaces)
        if body_element is not None:
            body_text = "".join(body_element.itertext())
            references = [ref.attrib.get("href", "") for ref in body_element.findall(".//akn:ref", namespaces)]
    ...
    def extract_all(self) -> None:
        ...
        extracted, errors, total = 0, 0, 0
        definitions_output_dir = Path("data/definitions_corpus")
        definitions_output_dir.mkdir(parents=True, exist_ok=True)

        for collection in self.collections:
            target = Path(definitions_output_dir / collection)
            for file in target.rglob("*.xml"):
                total += 1
                try:
                    definitions = self.parse_xml(file, collection)
                    if definitions:
                        extracted += 1
                        output_file = definitions_output_dir / collection / file.name.replace(".xml", ".tsv")
