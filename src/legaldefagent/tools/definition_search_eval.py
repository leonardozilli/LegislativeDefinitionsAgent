...
from ..utils import camelcase_to_spaces, setup_logging
...
def filter_existing_definitions(definiendum, definitions):
    return [
        ...
        if camelcase_to_spaces(definition.metadata["definiendum_label"]) != definiendum
    ][:10]
...
async def definition_search(
    ...
    if len(picked_definitions_ids) == 0:
        examples = list(set(definition.get("definition_text") for definition in relevant_definitions))
